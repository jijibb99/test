...
class preprocessing:
    ...
    def scale(self, x):
        ...
        pickle.dump(scale_quant, open(str("./scaler/scale_quant.sav"), 'wb'))
...
class modeling:
    ...
    #def grid_search_cv(self, name):
    #    grid_search = GridSearchCV(LGBMClassifier(), param_grid, cv=5, return_train_score=True)
    #    "lgb": (lambda: LGBMClassifier(boosting_type='gbdt'))
...
class measuring:
    ...
    def print_model(self, y_pred_prob, table=False):
        ...
        print("f1-score : ", round(f1_score(y_te, precision), 2))
...
if __name__ == "__main__":
    ...
    # load models
    y_pred_prob_vl, roc_thres_vl, pr_thres_vl, f_list_vl, loading_time_vl = model.load_cv_models(pwd_cv, vl=True,
                                                                                                 vl_index=vl_index)
    y_pred_prob_cv_te, roc_thres_cv_te, pr_thres_cv_te, f_list_cv_te, loading_time_cv_te = model.load_cv_models(pwd_cv)
    #y_pred_prob_ch_te, roc_thres_ch_te, pr_thres_ch_te, f_list_ch_te, loading_time_ch_te = model.load_chunck_models(
    #    pwd_chunck)
